"""Post-training set v10 = v8 sample + the targeted synthetic round (copy fidelity, from/to roles, Korean number words,
...
def main():
    ...
    ap.add_argument("--v8", required=True); ap.add_argument("--synth", default=""); ap.add_argument("--massive", default=""); ap.add_argument("--kosgd", default="")
    ap.add_argument("--out", required=True); ap.add_argument("--v8-sample", type=int, default=175000)
    ap.add_argument("--synth-weight", type=int, default=2); ap.add_argument("--massive-weight", type=int, default=2)
    ...
    v8 = load(a.v8); synth = load(a.synth); massive = load(a.massive, split="train"); kosgd = load(a.kosgd)
    ...
    extra_rows = []
    for src, weight in ((synth, a.synth_weight), (massive, a.massive_weight), (kosgd, 1)):
        for r in src:
    ...
    print(f"wrote {a.out}: {len(out)} rows (v8 {len(v8)}, synth {len(synth)}x{a.synth_weight}, massive {len(massive)}x{a.massive_weight}, kosgd {len(kosgd)}, distinct names {len({t['name'] for r in out for t in r['tools']})})")
