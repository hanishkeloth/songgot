...
const MODEL_VERSION = "2026-09-16-x-soup4";  // bumped by site/publish.py on every 12L publish so browsers refetch the new weights
...
//   songgotx  Songgot-X 0.8B, built on Qwen3.5-0.8B (Apache 2.0) and post-trained by us (three-way average + v13 continuation, 2026-09-16), 505 MB, desktop and Android
const MODELS = {
  ...
  songgotx: { url: "https://huggingface.co/palette-lab/songgot-x-0.8b/resolve/main/songgot-x-q4_k_m.gguf?v=" + MODEL_VERSION, label: "Songgot-X 0.8B · on Qwen3.5 base · 505 MB · 82.2%", short: "Songgot-X 0.8B · on an open base", mb: 505, n_ctx: 2048, n_batch: 64, format: "qwen35" },
};
...
(async () => {
  ...
  try {
    ...
    await wllama.loadModelFromUrl(MODEL_URL, { n_ctx: Number(params0.get("ctx")) || MODEL.n_ctx, n_batch: Number(params0.get("nb")) || MODEL.n_batch || 512, progressCallback: ({ loaded, total }) => { setBar(total ? loaded / total : null); setStatus(`downloading ${Math.round(100 * loaded / (total || 1))}%`, "busy"); } });
